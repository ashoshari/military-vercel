...
function ChartCard({
  ...
  const mergedOption = useMemo(() => {
    ...
    const optTooltipExtra =
      typeof chartTooltip.extraCssText === "string"
        ? String(chartTooltip.extraCssText).trim()
        : "";
    const darkTooltipExtra = [baseTip.extraCssText, optTooltipExtra]
      .filter(Boolean)
      .join("; ");
    const mergedTooltip = isDark
      ? {
          ...baseTheme.tooltip,
          ...chartTooltip,
          extraCssText: darkTooltipExtra || baseTip.extraCssText,
        }
      : {
          ...chartTooltip,
          ...
          extraCssText: optTooltipExtra
            ? `${optTooltipExtra}; ${lightTooltipExtra}`
            : lightTooltipExtra,
        };
    ...
    if (Array.isArray(merged.xAxis)) merged.xAxis = merged.xAxis.map(killSplit);
      ...
      merged.xAxis = killSplit(merged.xAxis as Record<string, unknown>);
    if (Array.isArray(merged.yAxis)) merged.yAxis = merged.yAxis.map(killSplit);
    else if (merged.yAxis)
      merged.yAxis = killSplit(merged.yAxis as Record<string, unknown>);
    ...
    if (!isDark && merged.series) {
      merged.series = (merged.series as Record<string, unknown>[]).map((s) => {
        ...
        if (ser.type === "gauge") {
          ...
          const lineStyle = (axisLine.lineStyle || {}) as Record<
            string,
            unknown
          >;
          return {
    ...
    const hasCartesianLineOrBar = seriesList.some(
      (s) => s.type === "bar" || s.type === "line",
    );
    ...
    if (hasCartesianLineOrBar) {
      ...
      const enhanceBarCartesianAxis = (
        ...
        const existingLS = (existingLine.lineStyle || {}) as Record<
          string,
          unknown
        >;
      ...
      if (Array.isArray(merged.xAxis)) {
        merged.xAxis = merged.xAxis.map((ax) =>
          pipeAxis(ax as Record<string, unknown>),
        );
      } else if (merged.xAxis) {
        merged.xAxis = pipeAxis(merged.xAxis as Record<string, unknown>);
      }
      if (Array.isArray(merged.yAxis)) {
        merged.yAxis = merged.yAxis.map((ax) =>
          pipeAxis(ax as Record<string, unknown>),
        );
      } else if (merged.yAxis) {
  ...
  const chartShell = (
    <>
      ...
      <div
        style={{
          height,
          overflow: panelOverflow === "visible" ? "visible" : undefined,
        }}
      >
        {chartEl()}
      </div>
