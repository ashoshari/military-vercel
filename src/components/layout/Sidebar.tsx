...
export default function Sidebar() {
    ...
    return (
        <AnimatePresence>
            {isShown && (
                <>
                    ...
                    <motion.aside
                        ...
                        transition={isOverlay ? { type: 'spring', damping: 28, stiffness: 300 } : undefined}
                        className={`flex flex-col z-50 sidebar-transition overflow-hidden ${isOverlay ? 'fixed top-0 right-0 h-screen lg:hidden' : 'sticky top-0 h-screen shrink-0'
                            }`}
                        style={{
                            width,
            ...
            {isSmallViewport && (
                <div className="px-4 py-3 border-b space-y-3" style={{ borderColor: 'var(--sidebar-border)' }}>
                    {/* Profile */}
                    <div className="flex items-center gap-3">
                        {user?.avatar ? (
                            <div className="relative w-9 h-9 rounded-full overflow-hidden shrink-0" style={{ border: '1px solid var(--sidebar-border)' }}>
                                <Image src={user.avatar} alt={user.nameAr || user.name} fill sizes="36px" style={{ objectFit: 'cover' }} />
                            </div>
                        ) : (
                            <div className="w-9 h-9 rounded-full flex items-center justify-center text-xs font-bold text-white shrink-0" style={{ background: 'var(--accent-green)' }}>
                                {user?.role === 'admin' ? 'م' : 'ض'}
                            </div>
                        )}
                        <div className="min-w-0">
                            <p className="text-sm font-semibold truncate" style={{ color: 'var(--text-primary)' }}>{user?.nameAr || 'مسؤول النظام'}</p>
                            <p className="text-[10px] truncate" style={{ color: 'var(--text-muted)' }}>{user?.department || 'مدير النظام'}</p>
                        </div>
                    </div>
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <button className="relative p-2 rounded-lg transition-colors" style={{ color: 'var(--text-muted)' }} type="button">
                            <Bell size={18} />
